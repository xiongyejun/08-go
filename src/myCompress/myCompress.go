...
func init() {
	...
	mt.doCompress = flag.Bool("c", false, "压缩文件")
	flag.PrintDefaults()
	flag.Parse()
}
...
func main() {
	...
	mt.fileName = os.Args[n-1]
	if _, err := os.Stat(mt.fileName); err != nil {
		fmt.Println(err)
		return
	}
...
func (me *myType) compress() (err error) {

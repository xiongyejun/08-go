...
type Layout struct {
	chatBox         *walk.TextEdit // *ui.Par //聊天窗口
	msgInBox        *walk.TextEdit //  *ui.Par //消息窗口
	editBox         *walk.TextEdit // *ui.Par // 输入框
	userNickListBox *walk.TextEdit //  *ui.List
	userNickList    []string
	userIDList      []string
	curUserIndex    int
...
func NewLayout(userNickList []string, userIDList []string, myName, myID string, msgIn chan wechat.Message, msgOut chan wechat.MessageOut, closeChan, autoReply chan int, logger *log.Logger) *Layout {
	...
	userNickListBox := new(walk.TextEdit)
	msgInBox := new(walk.TextEdit)
	chatBox := new(walk.TextEdit)
	editBox := new(walk.TextEdit)
	...
	userNickListBox.SetText(strings.Join(userNickList, "\r\n"))
